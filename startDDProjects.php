...
$projects_array_sql = array(
    $pidsArray['DATAMODEL']=>array(
        'variable_replacedby' => array(
            'query' => "SELECT CONCAT(a.record, ':', b.instance), CONCAT(a.value, ':', b.value) FROM (SELECT record,value FROM ".$module->getDataTable($pidsArray['DATAMODEL'])." WHERE project_id=".$pidsArray['DATAMODEL']." AND field_name = 'table_name') a JOIN (SELECT record, value, IFNULL(instance,1) as instance FROM ".$module->getDataTable($pidsArray['DATAMODEL'])." WHERE project_id=".$pidsArray['DATAMODEL']." AND field_name = 'variable_name') b  ON b.record=a.record ORDER BY a.value, b.instance",
            'autocomplete' => '1',
            'label' => ""
        ),
        'code_list_ref' => array(
            'query' => "select record, value from ".$module->getDataTable($pidsArray['CODELIST'])." where project_id = ".$pidsArray['CODELIST']." and field_name = 'list_name' order by value asc",
            'autocomplete' => '0',
            'label' => ""
        )
    ),
    $pidsArray['DATAMODELMETADATA']=>array(
        'index_tablename' =>  array (
            'query' => "SELECT a.record,   CONCAT(   max(if(a.field_name = 'table_name', a.value, NULL)),    '  ' ) as value  FROM ".$module->getDataTable($pidsArray['DATAMODEL'])." a  WHERE a.project_id=".$pidsArray['DATAMODEL']."  GROUP BY a.record  ORDER BY value",
            'autocomplete' => '0',
            'label' => ""
        ),
